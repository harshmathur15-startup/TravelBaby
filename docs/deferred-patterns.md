...
**Implementation:** Workflow signals write to CSV history files (e.g., pulse-history.csv). Extend with more dimensions and timestamps. No new infrastructure needed — just richer CSV output.
**Note:** Partially done — history CSV exists. Additional dimensions need tracking. Signal work handled by /pulse skill.
