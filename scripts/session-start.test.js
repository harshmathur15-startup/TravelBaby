/**
 * Tests for session-start.js
 *
 * Uses Node's built-in test runner (node:test) — no external dependencies.
 * Run: node --test scripts/session-start.test.js
 */
...
const SCRIPT_PATH = path.join(__dirname, 'session-start.js');
...
describe('session-start.js', () => {
  ...
  describe('when run in the project root (git repo)', () => {
    it('should output session context to stdout', () => {
      // Run in the actual project root which is a git repo
